{{ Form::open(array('url'=>$action, 'class'=>'form-horizontal','id'=>'saveform' , 'parsley-validate'=>'','novalidate'=>' ')) }}
				...
				@foreach($columns as $column)
					<tr class="" id="field-{{ $column->Field }}">
						...
						<td>
							...
							<a href="{{ URL::TO('config/tablefieldedit/'.$table.'?'.$info) }}" class="btn btn-xs btn-primary" onclick="SximoModal(this.href,'Edit New Column : {{$column->Field}} '); return false;" ><i class="fa fa-edit "></i></a>
							<a href="{{ URL::TO('config/tablefieldremove/'.$table.'/'.$column->Field) }}" rel="#field-{{ $column->Field }}" class="removedField  btn btn-xs btn-danger"><i class="fa fa-minus"></i></a>
						</td>
		...
		@else
		<a href="{{ URL::TO('config/tablefieldedit/'.$table) }}" class="btn btn-xs btn-info" onclick="SximoModal(this.href,'Add New Column'); return false;" ><i class="fa fa-plus"></i> Add New Field</a>
		@endif	
...
function showResponse(data)  {		
	...
	{
		window.location.href = '{{ URL::TO("config/tables") }}';

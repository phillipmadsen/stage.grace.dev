...
class BaseController extends Controller {
	...
	public function getExport( $t = 'excel')
	{
		...
		{			
		 	...
		 	$pdf = PDF::loadView('admin.module.utility.pdf', $content);
			return $pdf->download($this->data['pageTitle'].'.pdf');
	...
	function postDetailview( $mode = 'view')
	{
		...
		$md 						= explode(" ", Input::get('md') );
		$this->data['foreignKey']	= $md[3];
		$this->data['foreignVal']	= SiteHelpers::encryptID($md[4],true);
		$this->data['access']		= $this->access;	
		$this->data['primarykey'] 	= $this->info['key']; 
		if($mode =='form'){
			return View::make('admin.module.utility.detailform',$this->data);	
		} else {
			return View::make('admin.module.utility.detailview',$this->data);		
		}
	...
	public function getSearch()
	{
		...
		$this->data['tableForm'] 	= $this->info['config']['forms'];	
		$this->data['tableGrid'] 	= $this->info['config']['grid'];
		return View::make('admin.module.utility.search',$this->data);
	...
	function postQuicksave( $id =0)
	{
		...
		if ($validator->passes()) {	
			...
			unset($data['{key}']);
			$ID = $this->model->insertRow($data , Input::get($this->info['key']));
